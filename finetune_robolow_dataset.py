#!/usr/bin/env python3
"""
Fine-tune the transfer learning model using the large robolow-dataset in COCO format.
This script leverages the much larger dataset (6,921 training images) for better model performance.
"""
...
class RobolowCOCODataLoader:
    """Load and process Roboflow COCO format dataset for weapon detection"""
    ...
    def __init__(self, dataset_path, image_size=(224, 224)):
        ...
        self.train_annotations = os.path.join(dataset_path, 'train', '_annotations.coco.json')
        self.train_images_dir = os.path.join(dataset_path, 'train')
        self.val_annotations = os.path.join(dataset_path, 'valid', '_annotations.coco.json')
        self.val_images_dir = os.path.join(dataset_path, 'valid')
        
        print(f"Loading training data from {self.train_annotations}")
        print(f"Loading validation data from {self.val_annotations}")

    def load_coco_split(self, annotations_file, images_dir):
        """Load a COCO format dataset split"""
        ...
        print(f"Categories: {[cat['name'] for cat in categories.values()]}")
        
        return images, annotations, categories, images_dir

    def load_image(self, image_info, images_dir):
    ...
    def get_annotations_for_image(self, image_id, annotations):
    ...
    def create_detection_dataset(self, images, annotations, images_dir, max_boxes_per_image=5):
        ...
        for image_info in images.values():
            try:
                # Load and preprocess image
                image, original_size = self.load_image(image_info, images_dir)
                
                # Get annotations for this image
                image_annotations = self.get_annotations_for_image(image_info['id'], annotations)
        ...
        return np.array(dataset_images), np.array(targets)

    def load_train_val_data(self, max_boxes_per_image=5):
        """Load both training and validation data"""
        # Load training data
        print("\n📁 Loading training data...")
        train_images_dict, train_annotations, train_categories, train_dir = self.load_coco_split(
            self.train_annotations, self.train_images_dir
        )
        X_train, y_train = self.create_detection_dataset(
            train_images_dict, train_annotations, train_dir, max_boxes_per_image
        )
        
        # Load validation data
        print("\n📁 Loading validation data...")
        val_images_dict, val_annotations, val_categories, val_dir = self.load_coco_split(
            self.val_annotations, self.val_images_dir
        )
        X_val, y_val = self.create_detection_dataset(
            val_images_dict, val_annotations, val_dir, max_boxes_per_image
        )
...
def create_detection_model(input_shape=(224, 224, 3), max_detections=5, num_classes=1):
    ...
    model = Model(inputs, outputs, name='robolow_weapon_detection_model')
...
def create_callbacks(checkpoint_path, patience=20):
    ...
    callbacks = [
        ...
        tf.keras.callbacks.LearningRateScheduler(
            lambda epoch: 1e-3 * (0.95 ** epoch) if epoch < 30 else 1e-5 * (0.98 ** (epoch - 30)),
            verbose=0
        )
    ]
    return callbacks
...
def main():
    ...
    # Configuration
    DATASET_PATH = 'finetune-data/robolow-dataset'
    CHECKPOINT_PATH = 'models/robolow_transfer_learning_model.keras'
    INPUT_SIZE = (224, 224, 3)  # EfficientNet standard size
    MAX_DETECTIONS = 5
    BATCH_SIZE = 32  # Larger batch size for the bigger dataset
    EPOCHS = 80  # Fewer epochs due to much more data per epoch
    
    print("🚀 Starting Transfer Learning with Robolow Dataset...")
    
    # Load dataset
    print("\n📁 Loading Roboflow COCO dataset...")
    data_loader = RobolowCOCODataLoader(DATASET_PATH, image_size=INPUT_SIZE[:2])
    X_train, y_train, X_val, y_val = data_loader.load_train_val_data(max_boxes_per_image=MAX_DETECTIONS)
    
    print(f"\n📊 Dataset Statistics:")
    print(f"Training: {X_train.shape} images, {y_train.shape} targets")
    print(f"Validation: {X_val.shape} images, {y_val.shape} targets")
    ...
    # Create TensorFlow datasets for better performance with large dataset
    ...
    # First phase: Train detection head only
    phase1_epochs = int(EPOCHS * 0.6)  # 60% of epochs for head training
    ...
    callbacks_phase2 = create_callbacks('models/robolow_transfer_learning_phase2.keras', patience=15)
    ...
    # Save final model
    final_model_path = 'models/robolow_weapon_detection_final.keras'
    ...
    plt.tight_layout()
    plt.savefig('robolow_transfer_learning_history.png', dpi=150, bbox_inches='tight')
    plt.show()
    
    print("\n🎉 Transfer learning completed successfully!")
    print(f"Best model saved at: {CHECKPOINT_PATH}")
    print(f"Phase 2 model saved at: models/robolow_transfer_learning_phase2.keras")
...
def test_model():
    ...
    try:
        ...
        model = tf.keras.models.load_model('models/robolow_weapon_detection_final.keras', 
